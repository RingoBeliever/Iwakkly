...
if($_SERVER["REQUEST_METHOD"] =="POST") {
  ...
  if(isset($_POST["submit_add"])) {
    ...
    $new_stagename = htmlspecialchars($_POST["stagename"], ENT_QUOTES); //追加Id
    ...
    //SQL文作成
    if($error == "") {
      $sql = "INSERT INTO usersdata VALUES('$new_id', '$new_stagename', '$new_puzzle', '$new_main', '$new_element', '$new_dir', '$new_clear', '$new_block', '$new_maxBlocks')";
      $mysql->query($sql);
    }
  }
  ...
  if(isset($_POST["submit_upd"])) {
    ...
    //全角文字を半角に変換
    $new_id = mb_convert_kana($new_id, "as");
    $stagename = mb_convert_kana($stagename, "as");
    $new_puzzle = mb_convert_kana($new_puzzle, "as");
    $new_main = mb_convert_kana($new_main, "as");
    $new_element = mb_convert_kana($new_element, "as");
    $new_dir = mb_convert_kana($new_dir, "as");
    ...
    //SQL文作成
    if($error == "") {
      $sql = "UPDATE usersdata SET stagename = '$stagename', puzzle = '$puzzle', main = '$main', element = '$element', dir = '$dir', clear = '$clear', block = '$block', maxBlocks = '$maxBlocks' WHERE id = $id";
      $mysql->query($sql);
...
<form action = "<?=$_SERVER["PHP_SELF"]?>" method = "POST">
  <table border = "1" cellspacing = "0" cellpadding = "3" width = "100%" bordercolor = "#666666">
    ...
    <?php
    //テーブルデータから読み込む
    $mysql->query("SELECT * FROM usersdata ORDER BY id");
